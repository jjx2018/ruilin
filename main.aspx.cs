...
namespace AppBoxPro
{
    public partial class main : PageBase
    {
        ...
        private void LoadData()
        {
            ...
            //if (link != null)
            //{
            //    //link.Text = String.Format("AppBoxPro v{0}", GetProductVersion());
            //    link.Text = ConfigHelper.Title;
            //}
            stitle= ConfigHelper.Title;
        ...
        {
            //ShowNotify(DateTime.Now.ToString());
            string sql = "select count(*) from OrderHeader where IsConfirm=0 and CONVERT(varchar(100), InputerDate, 23)=CONVERT(varchar(100), GETDATE(), 23)";
            SQLHelper.DbHelperSQL.SetConnectionString("");
            if (int.Parse(SQLHelper.DbHelperSQL.GetSingle(sql, 30)) > 0)
            {
                PageContext.RegisterStartupScript(Window1.GetShowReference("~/OrderMag/showNewOrder.aspx", "新订单", Unit.Parse("1000"), Unit.Parse("500")));
            }
        }
